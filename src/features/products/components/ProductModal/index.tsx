...
const ProductModal = ({ productModal, setProductModal }: ProductModalProps) => {
  const { control, errors, handleSubmit, isLoading, categories, image, clear, openFilePicker, plainFiles } = useProductModal({
    productModal,
    setProductModal,
  })
  ...
  return (
    <Modal width="max-w-2xl" isOpen={productModal.isOpen}>
      ...
      <Modal.Body>
        <form className="flex flex-wrap *:flex-[1_1_250px] gap-2 p-4">
          ...
          <Textarea
            ...
            rows={5}
          />
          <ProductImage name="image" image={image} control={control} files={plainFiles} openFilePicker={() => openFilePicker()} />
        </form>
      </Modal.Body>
      <Modal.Footer>
