...
const CartItem = ({ id, img, price, quantity, title, selectedQuantity }: CartItemProps) => {
  const [itemQuantity, setItemQuantity] = useState({ label: selectedQuantity, value: selectedQuantity })
    ...
    >
      ...
      <div className="flex items-center justify-between flex-[2_1_250px]">
        <Select
          ...
          options={options}
        />
        <span className="text-lg  text-custom-slate font-semibold">{price.toLocaleString() + ' ' + 'ریال'}</span>
