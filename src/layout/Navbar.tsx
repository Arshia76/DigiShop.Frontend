  ...
  return (
    <header className="flex items-center bg-custom-light-gray justify-around shadow-lg py-4 top-0 sticky z-50">
      ...
      <div className="flex items-center gap-4">
        <div className="relative cursor-pointer me-6" onClick={() => navigate(Routes.CART)}>
          <span
        ...
        {user?.role === 'Admin' ? (
          <>
            <Button colour={'primary'} className="w-fit px-8" onClick={() => navigate(Routes.ADMIN.MAIN)}>
              پنل ادمین
            </Button>
            <Button className="w-fit px-8 bg-slate-800 text-white" onClick={() => navigate(Routes.PROFILE + '/' + user.id)}>
              پروفایل
            </Button>
          </>
        ) : user?.role === 'User' ? (
          <Button colour={'primary'} className="w-fit px-8" onClick={() => navigate(Routes.PROFILE + '/' + user.id)}>
            پروفایل
          </Button>
        ) : null}
        {isAuthenticated && (
          <Button colour={'secondary'} className="w-fit px-8" onClick={() => signout()}>
            خروج
          </Button>
        )}
      </div>
